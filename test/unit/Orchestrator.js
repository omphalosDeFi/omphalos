...
const MockOmphalosPolicy = artifacts.require('MockOmphalosPolicy.sol');
...
async function setupContracts () {
  ...
  deployer = accounts[0];
  user = accounts[1];
  mockPolicy = await MockOmphalosPolicy.new();
...
contract('Orchestrator', function (accounts) {
  ...
  describe('when transaction list is empty', async function () {
    ...
    it('should call rebase on policy', async function () {
      ...
      expect(fnCalled.args.instanceName).to.eq('OmphalosPolicy');
