...
const omphalosGrowth = new Stochasm({ min: -0.5, max: 2.5, seed: 'fragments.org' });

let omphalos, rebaseAmt, inflation, preRebaseSupply, postRebaseSupply;
...
async function checkBalancesAfterOperation (users, op, chk) {
  ...
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      _bals.push(await omphalos.balanceOf.call(users[u]));
    }
  }
  await op();
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      bals.push(await omphalos.balanceOf.call(users[u]));
    }
  }
  chk(_bals, bals);
}
...
async function checkBalancesAfterTransfer (users, tAmt) {
  await checkBalancesAfterOperation(users, async function () {
    await omphalos.transfer(users[1], tAmt, { from: users[0] });
...
async function exec () {
  ...
  let i = 0;
  do {
    await omphalos.rebase(i + 1, rebaseAmt, {from: deployer});
    postRebaseSupply = await omphalos.totalSupply.call();
    i++;

    console.log('Rebased iteration', i);
    console.log('Rebased by', (rebaseAmt.toString()), 'OMPL');
    ...
    const tAmt = (await omphalos.balanceOf.call(deployer));
    ...
    inflation = omphalosGrowth.next().toFixed(5);
    rebaseAmt = preRebaseSupply.mul(inflation).dividedToIntegerBy(1);
  } while ((await omphalos.totalSupply.call()).plus(rebaseAmt).lt(endSupply));
}
